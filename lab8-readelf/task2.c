...
void symbols(){
    ...
    Elf64_Ehdr* ehdr = (Elf64_Ehdr*) map_start; //header
    Elf64_Shdr* shdr = (Elf64_Shdr*)(map_start + ehdr->e_shoff); //sections
    Elf64_Shdr* sh_strtab = &shdr[ehdr->e_shstrndx];

    char* sh_strtab_abs = map_start + sh_strtab->sh_offset;

    int shnum = ehdr->e_shnum, i, j, print_index = 0;
    Elf64_Sym* symtable;

    printf("Index\tValue\t\t\tSection Index\tSection Name\t\tSymbol Name\n");
    for(i = 0 ; i < shnum ; ++i) {
        if (shdr[i].sh_type == SHT_SYMTAB) {
            symtable = (Elf64_Sym*) (map_start + shdr[i].sh_offset);
            break;
        }
    }

    if(symtable) {
        int len, padding = 24, sym_size = shdr[i].sh_size / shdr[i].sh_entsize;
        char* sh_name;
        char* sym_name;
        char* sh_tab;
...
int main(){
	void (*funcs[])() = {NULL,examine,sections,symbols,quit};

	int mod;
	while(1){
        menu();
		scanf("%d", &mod);
        if(mod == 0 || mod > 4) { //fail safe - illegal mod input
